  ...
  def dialogs
    ...
    # where if the responses array has only one response whose hash does not contain the key "text", it will just be one of those "there is no response, just hit enter to continue and do the actions".
    {
        ...
        "responses" => [
          {
            ...
            "audio"   => "audio file",
            "actions" => ["set_state('#{name}', 'next dialog', 'why investigating')", "do_dialog('#{name}')"]
          },
          {
            "text"    => "Just visiting the ol' town",
            "audio"   => "audio file",
            #"actions" => ["set_state('#{name}', 'next dialog', 'why investigating')", "do_dialog('#{name}')"]
            "actions" => ["set_state('#{name}', 'next dialog', 'thats nice')", "do_dialog('#{name}')"]
          }
        ]
      },
      "why investigating" => {
        ...
        "audio"     => "audio file",
        "responses" => [{"actions" => ["set_state('#{name}', 'next dialog', 'I told you')"]}]
      },
      "I told you" => {
      ...
      "thats nice" => {
        ...
        "audio" => "audio file",
        "responses" => [{"text"=>"Ok, nice seeing you again.", "audio" => "audio file", "actions" => ["set_state('#{name}', 'next dialog', 'have to go')"]}]
      },
      "have to go" => {
        "text"      => "Like I said, I have to be going.  Bye now",
        "audio"     => "audio file",
        "responses" => [{"actions" => ["set_state('#{name}', 'next dialog', 'have to go')"]}]
      }
    }
  end
